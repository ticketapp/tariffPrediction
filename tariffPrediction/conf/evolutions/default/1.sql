...
CREATE TABLE barCodesWithUrl(
  ...
  userId                    UUID REFERENCES users(userId) NOT NULL,
  eventId                   INT REFERENCES salableEvents(eventId) NOT NULL
);
...
CREATE UNIQUE INDEX followedEventsByUserIdIndex ON followedEvents (userId, eventId);
CREATE UNIQUE INDEX followedEventsByGuestUserIndex ON followedEvents (guestUserIP, eventId);
...
CREATE TABLE eventscounts(
  eventid               VARCHAR(63) PRIMARY KEY REFERENCES events(facebookId) NOT NULL,
